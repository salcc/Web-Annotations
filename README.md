...
- This extension targets regular web pages, not PDF viewer pages.
- When reloading/updating the extension during development, export your data first as a backup.
- Cross-browser support is configured through both `background.service_worker` and `background.scripts` in the manifest.
- Firefox/Safari compatibility is expected but currently best-effort (not exhaustively tested).
